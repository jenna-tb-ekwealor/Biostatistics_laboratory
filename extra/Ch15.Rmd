...
<p style="text-align: center;">[*Chapter 15*]{style="font-size:120%; font-weight:400"}</p>
...
- If you haven’t already, download this week's dataset, *spatial.zip*. It contains several `.laz` format point cloud datasets that you will use in this lesson.
...
<p style="text-align: center;">[Coordinate Reference System]{style="color:darkblue; font-size:300%; font-weight:400"}</p>
...
![The spatial extent of vector data which you will learn later in this chapter. Notice that the spatial extent represents the rectangular area that the data cover. Thus, if the data are not rectangular (i.e. points OR an image that is rotated in some way) the spatial extent covers portions of the dataset where there are no data. Image Source: National Ecological Observatory Network (NEON)](images/spatial-extent.png)
...
Create a map that shows the madera roads layer and the `sjer` plot locations.  Use the `sjer_crop` boundary (`sjer_crop.shp`) to crop your map. Recall using `extent()` to find the extent of a layer. 
...
*IMPORTANT:* be sure that all of the data are within the same `extent` and `crs` of the `sjer_crop` layer. This means that you may have to crop and reproject your data prior to plotting it!
